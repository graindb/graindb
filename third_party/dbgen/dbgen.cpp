...
string get_default_jo(string jo_name) {
	if (jo_name == "EMPTY") {
		return "{}";
	}
	string jo_file_name = "third_party/dbgen/jos/default/" + jo_name + ".json";
...
string get_optimized_jo(string jo_name) {
	if (jo_name == "EMPTY") {
		return "{}";
	}
	string jo_file_name = "third_party/dbgen/jos/optimized/" + jo_name + ".json";
	auto file_system = make_unique<FileSystem>();
	if (!file_system->FileExists(jo_file_name)) {
		return "";
	}
