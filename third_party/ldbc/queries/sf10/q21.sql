             ...
             search_graph(link, depth, path) AS (
                     ...
                     (WITH sg(link, depth) as (select * from search_graph)
                      ...
                      WHERE x.link = k_person1id
                        and not exists (select * from sg y where y.link = 21990232562871::bigint)
                        and not exists ( select * from sg y where y.link=k_person2id)
                     )
             ),
             paths(pid, path) AS (
                 SELECT row_number() OVER (), path FROM search_graph where link = 21990232562871::bigint
     ),
    edges(id, e) AS (
