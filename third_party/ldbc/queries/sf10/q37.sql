...
WITH RECURSIVE friends AS (
    ...
    WHERE 1 = 1
      ...
      -- filter
      AND NOT ARRAY[ROW(k.k_person1id, k.k_person2id)
   , ROW(k.k_person2id, k.k_person1id)] && f.path
-- stop condition
    AND coalesce (array_length(f.path
   , 1)
   , 0)
   < :maxPathDistance
    )
...
ORDER BY messageCount DESC, t.t_name, m.friendid LIMIT 100
;
