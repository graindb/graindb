...
select p_personid,
       ...
       (case
            when exists(select 1 from knows where k_person1id = 5132 and k_person2id = p_personid) then 0
            else 1 end) as isnew
from (select l_personid, max(l_creationdate) as l_creationdate
      from likes,
           message
      where m_messageid = l_messageid
        and m_creatorid = 5132
      group by l_personid
