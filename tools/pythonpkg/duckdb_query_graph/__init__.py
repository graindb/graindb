...
def generate_html(parsed_json):
	...
	def timing_to_str(timing):
		timing_str = "%.5f" % (timing,)
		if timing_str == "0.00000":
			return "0.0"
		# if "e-" in timing_str:
		# 	timing_str = "0.0"
		return timing_str
...
"""
		# add the total time
		meta_info += """<tr class="metainfo mainphase"><th style="font-weight:bold;">Total Time</th><th style="font-weight:bold;">${TOTAL_TIME}ms</th><th></th></tr>""".replace("${TOTAL_TIME}", timing_to_str(total_time))
		# add the execution phase
		meta_info += """<tr class="metainfo mainphase"><th>Execution</th><th>${EXECUTION_TIME}ms</th><th>${PERCENTAGE}</th></tr>""".replace("${EXECUTION_TIME}", str(execution_time)).replace("${PERCENTAGE}", to_percentage(total_time, execution_time))
		execution_nodes = node_timings.keys()
		execution_nodes = sorted(execution_nodes, key=lambda x: -node_timings[x])
		for node in execution_nodes:
			meta_info += '<tr class="metainfo subphase"><th>${NODE}</th><th>${NODE_TIME}ms</th><th>${PERCENTAGE}</th></tr>'.replace("${NODE}", node).replace("${NODE_TIME}", timing_to_str(node_timings[node])).replace("${PERCENTAGE}", to_percentage(total_time, node_timings[node]))
		...
		for key in keys:
			...
			meta_info += "</th>"
			meta_info += '<th>' + timing_to_str(timings[key]) + "ms" + "</th>"
	...
	def convert_json(node):
		...
		global current_tone
		result = '{ "text": {'
		result += '"name": "' + node["name"] + ' (' + str(node["timing"]) + 'ms)",\n'
		title = ""
