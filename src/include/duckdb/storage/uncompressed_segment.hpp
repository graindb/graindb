...
public:
	...
	block_id_t block_id;
	//! The block buffer pointer this segment relates to
	data_ptr_t block_buffer;
...
public:
	...
	virtual void FetchRow(ColumnFetchState &state, Transaction &transaction, row_t row_id, Vector &result,
	                      idx_t result_idx) = 0;
	virtual bool FilterFetchRow(ColumnFetchState &state, Transaction &transaction, row_t row_id,
	                            vector<TableFilter> &filters, Vector &result, idx_t result_idx) = 0;
