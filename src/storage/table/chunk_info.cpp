...
idx_t ChunkDeleteInfo::GetSelVector(Transaction &transaction, SelectionVector &sel_vector, idx_t max_count) {
	...
	for (idx_t i = 0; i < max_count; i++) {
		auto index = sel_vector.get_index(i);
		if (!UseVersion(transaction, deleted[index])) {
			sel_vector.set_index(count++, index);
		}
	}
	return count;
}
...
idx_t ChunkInsertInfo::GetSelVector(Transaction &transaction, SelectionVector &sel_vector, idx_t max_count) {
	...
	for (idx_t i = 0; i < max_count; i++) {
		auto index = sel_vector.get_index(i);
		if (UseVersion(transaction, inserted[index]) && !UseVersion(transaction, deleted[index])) {
			sel_vector.set_index(count++, index);
		}
	}
	return count;
