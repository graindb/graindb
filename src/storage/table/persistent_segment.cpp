...
bool PersistentSegment::FilterFetchRow(ColumnFetchState &state, Transaction &transaction, row_t row_id,
                                       vector<TableFilter> &filters, Vector &result, idx_t result_idx) {
	return data->FilterFetchRow(state, transaction, row_id - this->start, filters, result, result_idx);
}
