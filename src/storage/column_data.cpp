...
bool ColumnData::FilterFetchRow(ColumnFetchState &state, Transaction &transaction, row_t row_id,
                                vector<TableFilter> &filters, Vector &result, idx_t result_idx) {
	auto segment = (TransientSegment *)data.GetSegment(row_id);
	segment->FilterFetchRow(state, transaction, row_id, filters, result, result_idx);
	return true;
}
