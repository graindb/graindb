...
void QueryProfiler::EndOperator(DataChunk &chunk) {
	...
	op.End();
	auto node = tree_map[execution_stack.top()];
	auto phy_op = execution_stack.top();
	node->extra_info = phy_op->ExtraRenderInformation();
...
static string ToJSONRecursive(QueryProfiler::TreeNode &node) {
	...
	if (node.cardinality != 0) {
		result += "\"cardinality\":\"" + to_string(node.info.elements) + "/" + to_string(node.cardinality) + "\",\n";
	} else {
...
unique_ptr<QueryProfiler::TreeNode> QueryProfiler::CreateTree(PhysicalOperator *root, idx_t depth) {
	...
	node->depth = depth;
	if (root->type == PhysicalOperatorType::SEQ_SCAN) {
		auto root_op = reinterpret_cast<PhysicalTableScan *>(root);
		node->cardinality = root_op->table.info->cardinality;
	}
